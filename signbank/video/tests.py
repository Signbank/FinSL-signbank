...
class VideoTests(unittest.TestCase):
    ...
    def test_Video_create(self):
        ...
        self.assertEquals(
            os.path.dirname(vid.videofile.name), settings.VIDEO_UPLOAD_LOCATION)
        ...
        self.assertFalse(os.path.exists(
            vid.videofile.path), "vidfile still exists after delete at %s" % (vid.videofile.path,))
    ...
    def test_Video_poster_path(self):
        ...
        self.assertTrue(
            os.path.exists(poster_abs), "poster image %s is missing" % (poster_abs,))

        # do it again should give the same result, but won't have created the
        # file
