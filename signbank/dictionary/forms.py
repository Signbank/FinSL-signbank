...
class UserSignSearchForm(forms.Form):
    ...
    glossQuery = forms.CharField(label='Glosses containing', max_length=100,
                                 required=False, widget=forms.TextInput(attrs={'class': 'form-control'}))
    query = forms.CharField(label='Translations containing', max_length=100,
                            required=False, widget=forms.TextInput(attrs={'class': 'form-control'}))
    category = forms.ChoiceField(label='Search', choices=CATEGORY_CHOICES,
...
class GlossSearchForm(forms.ModelForm):
    ...
    defspublished = forms.ChoiceField(
        label="All Definitions Published", choices=YESNOCHOICES)
    ...
    relation = forms.CharField(
        label='Search for gloss of related signs', widget=forms.TextInput(attrs=ATTRS_FOR_FORMS))
    relationToForeignSign = forms.CharField(
        label='Search for gloss of foreign signs', widget=forms.TextInput(attrs=ATTRS_FOR_FORMS))
    morpheme = forms.CharField(
        label='Search for gloss with this as morpheme', widget=forms.TextInput(attrs=ATTRS_FOR_FORMS))

    phonOth = forms.CharField(
        label='Phonology other', widget=forms.TextInput())

    hasRelationToForeignSign = forms.ChoiceField(label='Related to foreign sign or not', choices=[
                                                 (0, '---------'), (1, 'Yes'), (2, 'No')], widget=forms.Select(attrs=ATTRS_FOR_FORMS))
    hasRelation = forms.ChoiceField(
        label='Type of relation', choices=RELATION_ROLE_CHOICES, widget=forms.Select(attrs=ATTRS_FOR_FORMS))
    hasMorphemeOfType = forms.ChoiceField(
        label='Has morpheme type', choices=MORPHEME_ROLE_CHOICES, widget=forms.Select(attrs=ATTRS_FOR_FORMS))

    # ,widget=forms.Select(attrs=ATTRS_FOR_FORMS));
    repeat = forms.ChoiceField(
        label='Repeating Movement', choices=NULLBOOLEANCHOICES)
    # ,widget=forms.Select(attrs=ATTRS_FOR_FORMS));
    altern = forms.ChoiceField(
        label='Alternating Movement', choices=NULLBOOLEANCHOICES)

    isNew = forms.ChoiceField(
        label='Is a proposed new sign', choices=NULLBOOLEANCHOICES, widget=forms.Select(attrs=ATTRS_FOR_FORMS))
    inWeb = forms.ChoiceField(
        label='Is in Web dictionary', choices=NULLBOOLEANCHOICES, widget=forms.Select(attrs=ATTRS_FOR_FORMS))
    definitionRole = forms.ChoiceField(
        label='Note type', choices=DEFN_ROLE_CHOICES, widget=forms.Select(attrs=ATTRS_FOR_FORMS))
    definitionContains = forms.CharField(
        label='Note contains', widget=forms.TextInput(attrs=ATTRS_FOR_FORMS))
...
class MorphologyForm(forms.ModelForm):
    ...
    role = forms.ChoiceField(label='Type', choices=build_choice_list(
