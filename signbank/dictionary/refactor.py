...
def refactor_glosses():
    ...
    for lex in lmodels.Sign.objects.all():
        ...
        for field in makeFields('english', 12):
            ...
            if translation != '':
                # make a keyword..
                (kwd, created) = models.Keyword.objects.get_or_create(
                    text=translation)
        ...
        if text != '':
            ...
            definition = newsign.definition_set.create(
                text=text, role='popexplain', count=1)

    # two keywords have bad characters in them
    # find them without mentioning the offending characters
    k = models.Keyword.objects.filter(
        text__startswith="Mont").exclude(text="Montreal")[0]
...
def refactor_relations():
    ...
    for lex in lmodels.Sign.objects.all():
        ...
        copy_relations(
            lex, newsign, 'var', ['b', 'c', 'd', 'e', 'f'], 'variant')
        ...
        if other != '':
            try:
                ...
                relation = models.Relation(
                    source=newsign, target=othergloss, role=role)
                relation.save()
            except:
...
def copy_definitions(lex, newsign, field, N, role):
    ...
    for count in range(1, N + 1):
        ...
        if text != '':
            ...
            definition = newsign.definition_set.create(
                text=text, role=role, count=count)
...
def build_homosign_relation():
    ...
    glosses = models.Gloss.objects.filter(
        sense__in=[1, 2, 3, 4, 5, 6]).order_by("sn")
    ...
    for gloss in glosses:
        if gloss.sense == 1:
            ...
            rel = models.Relation(
                source=gloss, target=last_one, role="homophone")
            rel.save()
        else:
