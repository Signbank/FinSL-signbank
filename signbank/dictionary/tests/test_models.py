...
from signbank.dictionary.models import (Dataset, Dialect, FieldChoice, Gloss,
                                        GlossTranslations, Keyword, Language,
...
class GlossTestCase(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            username="test", email=None, password=None)
        # Migrations have id=1 already
        self.language = Language.objects.create(
            name="glang", language_code_2char="gl", language_code_3char="gla")
    ...
    def test_unique_together(self):
        ...
        new_dataset = Dataset.objects.create(
            name="testdataset2", signlanguage=self.signlanguage)
    ...
    def test_idgloss(self):
        ...
        weird_chars = ("äöåÄÖÅ¨^~'* ´`üÜÿŸëêËÊ€$#",
                       "ЁЂЃЄЅІЇЌЍЎЏАБВДЖИКОПРСТФХЦЧЩЫ", "؟ الستارود أي بعد, معاملة بيو",)
    ...
    def test_idgloss_dataset(self):
        ...
        with self.assertRaises(IntegrityError):
            Gloss.objects.create(idgloss="testgloss7",
...
class DatasetTestCase(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            username="testdata", email=None, password=None)
        # Migrations have id=1 already
        self.signlanguage = SignLanguage.objects.create(
            pk=3, name="slang", language_code_3char="tst")
        self.dataset = Dataset.objects.create(
            name="dataset", signlanguage=self.signlanguage)
...
class GlossTranslationsTestCase(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            username="testglosstrans", email=None, password=None)
        ...
        (self.dataset, created) = Dataset.objects.get_or_create(
            name="dataset52", signlanguage=self.signlanguage)
        ...
        self.translations_grouped_keywords = [
            "cat", "dog", "lion", "crocodile", "mouse"]
    ...
    def test_get_keywords(self):
        """Test get_keywords()"""
        # Simple format
        self.assertEqual(self.glosstranslations.get_keywords(),
                         self.translations_keywords)
        # Grouped format
        self.assertEqual(self.glosstranslations_grouped.get_keywords(
        ), self.translations_grouped_keywords)
    ...
    def test_get_keywords_duplicates(self):
        ...
        self.assertEqual(self.glosstranslations_duplicates.get_keywords_unique(
        ), self.translations_duplicates_keywords)
    ...
    def test_has_duplicates(self):
        ...
        self.assertTrue(
            self.glosstranslations_grouped_duplicates.has_duplicates())
    ...
    def test_save(self):
        ...
        trans = Translation.objects.filter(
            gloss=self.gloss, language=self.language1)
        # Verify that these Translation objects have been deleted.
        self.assertFalse("elephant" and "ant" in [
                         str(x.keyword) for x in trans])
        # Verify that these Translation objects still exist.
        self.assertTrue("squirrel" and "magpie" in [
                        str(x.keyword) for x in trans])
...
class TranslationTestCase(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            username="testtrans", email=None, password=None)
        self.language = Language.objects.create(
            name="mylang", language_code_2char="ml", language_code_3char="myl")
        # Migrations have id=1 already for a SignLanguage
        self.signlanguage = SignLanguage.objects.create(
            pk=5, name="signlang", language_code_3char="sla")
        self.dataset = Dataset.objects.create(
            name="dataset", signlanguage=self.signlanguage)
...
class RelationToForeignSignTestCase(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            username="testrel", email=None, password=None)
        self.signlanguage = SignLanguage.objects.create(
            pk=6, name="lala", language_code_3char="lal")
        self.dataset = Dataset.objects.create(
            name="relset", signlanguage=self.signlanguage)
...
class FieldChoiceTestCase(TestCase):
    def setUp(self):
        self.fieldchoice = FieldChoice.objects.create(
            field="field", english_name="mychoice", machine_value=1)
...
class MorphologyDefinitionTestCase(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            username="usermorph", email=None, password=None)
        self.signlanguage = SignLanguage.objects.create(
            pk=11, name="definitive", language_code_3char="def")
        self.dataset = Dataset.objects.create(
            name="morphdata", signlanguage=self.signlanguage)
        ...
        self.gloss2 = Gloss.objects.create(idgloss="morhp-gloss2", dataset=self.dataset, created_by=self.user,
                                           updated_by=self.user)
        self.fieldchoice = FieldChoice.objects.create(
            field="newfield", english_name="nice name", machine_value=2)
...
class FunctionsTestCase(TestCase):
    def setUp(self):
        ...
        f1 = FieldChoice.objects.create(
            field=self.field, english_name="choice1", machine_value=1)
        f2 = FieldChoice.objects.create(
            field=self.field, english_name="choice_another", machine_value=2)
        f3 = FieldChoice.objects.create(
            field=self.field, english_name="full-of-choices", machine_value=3)
        self.choices = []
